...
class mpu9250(object):
    def __init__(self, bus=1):
        ...
        sleep(0.1) #give it all a chance to get online
    ...
    def setSlaveToWrite(self, address=None):
        ...
        self.bus.write_byte_data(MPU9250_ADDRESS, I2C_SLV0_ADDR, address) 

    # Write in slave
    def writeSlave(self, register, value, this_sleep = 0):
        self.bus.write_byte_data(MPU9250_ADDRESS, I2C_SLV0_REG, register)
        self.bus.write_byte_data(MPU9250_ADDRESS, I2C_SLV0_D0, value)
        self.bus.write_byte_data(MPU9250_ADDRESS, I2C_SLV0_CTRL, 0x80)
    ...
    def setSlaveToRead(self, address=None):
        ...
        self.bus.write_byte_data(MPU9250_ADDRESS, I2C_SLV0_ADDR, address | 0x80) 

    # Read from slave
    def readSlave(self, register, num_bytes=6):
        self.bus.write_byte_data(MPU9250_ADDRESS, I2C_SLV0_REG, register)
        self.bus.write_byte_data(MPU9250_ADDRESS, I2C_SLV0_CTRL, 0x80 + num_bytes)
        data = []
        for i in range(0,num_bytes):
            data.append(self.bus.read_byte_data(MPU9250_ADDRESS, I2C_SLV_DI + i))
        return data
